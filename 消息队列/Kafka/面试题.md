...
- ISR (In-Sync Replicas)：所有与 Leader 副本**保持一定程度同步**的Replica(包括 Leader 副本在内)组成 ISR。
- OSR (Out-of-Sync Replicas)：与 Leader 副本**同步滞后过多的** Replica 组成了 OSR。
...
![img](https://secure2.wostatic.cn/static/3MRtwhVQgztJau8ScLXXcH/image.png?auth_key=1701006703-qgH2uP2i3TBCStogWoPxCP-0-1d878db31eca01e42f1c312ada50741a)
...
![img](https://secure2.wostatic.cn/static/4tpLu3b5J4JL1NFeG1SoP2/image.png?auth_key=1701006703-pr3DsUq56qvh9rNeRMf3Tu-0-cdba5b81bc7462c15aed218971fe67f3)

如上图：三个分区副本都是 ISR集合当中的，最小的 LEO 为 3，就代表分区的 HW 为3，所以当前分区只能消费到 0~2 之间的三条数据，如下图

![img](https://secure2.wostatic.cn/static/dasDZkGTfVjbZ4J8Ec5KJN/image.png?auth_key=1701006703-uqNcXm3peA5KUAuGi5J7PK-0-de1b4b13c7557f3d7c8704ff7871b701)
...
![img](https://secure2.wostatic.cn/static/6uFAJ93cMqyYH9p6NiqJj9/image.png?auth_key=1701006703-w2EfWNWH3tTbtwryed9q32-0-c9bf864a065617bca6aa390076810a27)
...
![img](https://secure2.wostatic.cn/static/b7tHRVSrpYjTMRtCUGxg8S/image.png?auth_key=1701006703-6YK2U5fMKgpBqyR2rMFgmM-0-e561d0703eef4a9d8f2212233fddfdce)
...
![img](https://secure2.wostatic.cn/static/dozP6WGSbxYQueszmFfiog/image.png?auth_key=1701006703-2kYJvnrG724VW7ZifPhsW9-0-92ca1171ea614a69b1a8815e15170d84)
...
![img](https://secure2.wostatic.cn/static/mqR5Z2gjmfNBvtZRY4UeMp/image.png)
...
![img](https://secure2.wostatic.cn/static/t5v2fRvSs1fNCG672xawpB/image.png)

2）问题2：当集群新增 broker 时，只有新的主题分区会分配在该 broker 上，而老的主题分区不会分配在该 broker 上，就造成了**老节点和新节点之间的负载不均衡**。
...
![img](https://secure2.wostatic.cn/static/w85rzWizxisHYMyW11Hbiq/image.png)
...
![img](https://secure2.wostatic.cn/static/uoDuw9TbG23pJkLMcrzWpr/image.png)
...
假设消费组内有3个消费者：**C0、C1、C2**它们都订阅了4个主题：**t0、t1、t2、t3**并且每个主题有2个分区，也就是说整个消费组订阅了，**t0p0、t0p1、t1p0、t1p1、t2p0、t2p1、t3p0、t3p1** 这8个分区最终的分配结果如下：
...
举例： 同样消费组内有3个消费者：**C0、C1、C2**集群中有3个主题 **t0、t1、t2**这3个主题分别有 **1、2、3**个分区也就是说集群中有 **t0p0、t1p0、t1p1、t2p0、t2p1、t2p2** 这6个分区消费者**C0订阅了主题t0，消费者C1订阅了主题t0和t1，消费者C2订阅了主题t0、t1和t2**如果此时采用RoundRobinAssignor策略：
...
![img](https://secure2.wostatic.cn/static/bwvvGckk9kDSfJYJbMVwAc/image.png)
...
![img](https://secure2.wostatic.cn/static/8nJm4DeX2LMUzZUYQQLVoE/image.png)
