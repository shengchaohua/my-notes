...
HyperLogLog 是一种用来的估计集合基数的数据结构，简称HLL，可以用于判断一个元素是否存在于海量数据的集合中。虽然SET数据结构可以用来实现相同的任务，但是SET数据结构需要保存所有元素，对于海量数据场景，会消耗大量的内存。

作为一种概率数据结构，HyperLogLog以完美的准确性换取高效的空间利用率。HyperLogLog实现了最多使用12 KB的内存，并提供小于1%（大约为0.81%）的标准错误概率。如果HyperLogLog中的元素较少，需要的内存也会少得多。
...
-   PFADD：用于添加一个元素到HyperLogLog统计中。
-   PFCOUNT：用于获取HyperLogLog中的唯一元素个数（近似值）。
-   PFMERGE：用执行多个HyperLogLog之间的联合操作。
